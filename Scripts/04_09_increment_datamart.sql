...
dwh_delta AS (
    ...
    SELECT     
            ...
            dc.craftsman_id AS craftsman_name,
...
dwh_update_delta AS (
    -- Шаг 3
    SELECT     
            dd.exist_craftsman_id AS craftsman_id -- напишите код здесь
            FROM dwh_delta dd 
                WHERE dd.exist_craftsman_id IS NOT NULL    -- напишите код здесь
),
...
update_delta AS (
    -- Шаг 7
update dwh.craftsman_report_datamart as updates
SET
...
SELECT 
    GREATEST(COALESCE(MAX(craftsman_load_dttm), MAX(customers_load_dttm), MAX(products_load_dttm), NOW())) AS load_dttm
        FROM dwh_delta
)
